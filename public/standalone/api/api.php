...
/* hash djb2 -> base36, identik dengan util.js di frontend */
function djb2(string $s): string {
  $h = 5381; $n = strlen($s);
  for ($i = 0; $i < $n; $i++) $h = ($h * 33 + ord($s[$i])) & 0xFFFFFFFF;
...
function seed_defaults(PDO $db): void {
  ...
  if ($n === 0) {
    ...
    $db->prepare('INSERT INTO akun (username,nama,peran,hash,salt) VALUES (?,?,?,?,?)')
       ->execute(['admin', 'Administrator', 'admin', $hash, $salt]);
  }
}
...
/* foto: simpan dataURL ke uploads/ agar payload bootstrap ringan */
...
try {
  ...
  switch ($action) {
    ...
    case 'save': {
      ...
      } else { $db->rollBack(); fail('Tabel tidak dikenal: ' . $table); }
